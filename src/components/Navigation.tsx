...
import { Shield, MessageSquare, Hotel, User, LogOut, Code2, Globe, KeyRound, Copy } from 'lucide-react';
      ...
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-3 sm:px-6 lg:px-8">
        ...
        >
          ...
          <div>
            <div className="flex items-center space-x-2">
              <span className="text-sm sm:text-base font-bold tracking-tight text-zinc-900">
                Agent Concierge
              </span>
        ...
        <div className="flex items-center gap-1 rounded-lg bg-zinc-100 p-1 border border-zinc-200/80">
          <button
            ...
            onClick={() => setActiveTab('chat')}
            className={`flex items-center gap-1.5 rounded-md px-2.5 sm:px-4 py-1.5 text-xs font-semibold transition-all cursor-pointer ${
              activeTab === 'chat'
          ...
          >
            ...
            <span className="hidden md:inline">Chat do Concierge</span>
          ...
          >
            ...
            <span className="hidden md:inline">Demo Site WordPress</span>
            <span className="md:hidden">Demo WP</span>
          </button>
        </div>

        {/* Right Section: Snippets Modal Button & Admin Access Link */}
        <div className="flex items-center space-x-2">
          
          {/* Prominent Code Snippets Modal Button */}
          <button
            id="btn-open-embed-modal"
            type="button"
            onClick={onOpenEmbedModal}
            className="flex items-center gap-1.5 rounded-xl border border-zinc-300 bg-white px-2.5 sm:px-3.5 py-1.5 text-xs font-bold text-zinc-800 hover:bg-zinc-900 hover:text-white hover:border-zinc-900 transition-all shadow-xs cursor-pointer"
            title="Abrir janela de códigos e snippets de integração"
          >
            <Code2 className="h-3.5 w-3.5 text-zinc-700 group-hover:text-white" />
            <span>Códigos & Snippets</span>
          </button>
          ...
          <button
            ...
            className={`flex items-center gap-1.5 rounded-xl px-2.5 sm:px-3.5 py-1.5 text-xs font-bold transition-all shadow-xs cursor-pointer border ${
              activeTab === 'admin'
          ...
          {isAdminAuthenticated && activeTab === 'admin' && (
            <button
              ...
              className="flex items-center space-x-1 rounded-xl border border-zinc-200 bg-white px-2.5 py-1.5 text-xs text-zinc-600 hover:bg-rose-50 hover:text-rose-600 hover:border-rose-200 transition-colors cursor-pointer"
          ...
          {activeTab === 'chat' && (
            <button
              ...
              className="hidden xl:flex items-center space-x-1.5 rounded-xl border border-zinc-200 bg-zinc-50 px-2.5 py-1.5 text-xs text-zinc-700 hover:bg-zinc-100 transition-colors cursor-pointer"
